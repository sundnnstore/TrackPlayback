...
namespace TrackPlayback
{
    public partial class frmLoading : Form
    {
        ...
        string savPath = System.Windows.Forms.Application.StartupPath + @"\busmonitor";
        ...
        private void GetLinesInfo()
        {
            try
            {
                ...
                string queryLineStr = "SELECT B.lineID, B.lineName, B.lineID2 \r"
                                    + "FROM TB_User_Lines A INNER JOIN TB_Lines B ON B.lineID = A.lineID AND B.stopsign = 0 \r"
                                    + "WHERE A.userCode = '" + usercode + "' \r"
                                    + "ORDER BY B.lineName ";
                ...
                for (int k = 0; k < CLineList.LineInfo.Count; k++)
                {
                    ...
                    pgbInfo.Position = (int)(20 + 10 * ((double)k / CLineList.LineInfo.Count));
                    this.Refresh();

                    string queryBusStr = "SELECT B.busID, B.busID2, B.busNumber, B.plateNumber, B.busSIMNO, NVL(C.equipType,0) AS equipType, NVL(C.busCode,B.busNumber) AS busCode \r"
                                        + "FROM TB_Line_Buses A INNER JOIN TB_Buses B ON B.busID = A.busID AND B.stopsign = 0 \r"
                                        + "LEFT JOIN TB_BusEquipType C ON C.busID2 = B.busID2 \r"
                                        + "WHERE A.lineID = '" + CLineList.LineInfo[k].LineID + "' \r"
        ...
        private void GetTrackInfo()
        {
            for (int j = 0; j < CLineList.LineInfo.Count; j++)
            {
                ...
                pgbInfo.Position = (int)(50 + 50 * ((double)j / CLineList.LineInfo.Count));
                this.Refresh();

                try
                {
                    //获取线路下上下行站点数据
                    string queryStationStr = "SELECT a.STATION_DIREC AS direct, a.STATION_POS as position, a.STATIONID, b.STATIONNAME, b.STATION_LON, b.STATION_LAT, b.STATIONID2 \r"
                                            + "FROM TB_LINE_CASES c \r"
                                            + "INNER JOIN TB_LINE_STATIONS a ON a.LINEID = c.LINEID AND a.CASEID = c.CASEID \r"
                                            + "INNER JOIN TB_STATIONS b ON b.STATIONID = a.STATIONID AND b.stopsign = 0 \r"
                                            + "WHERE c.LINEID = '" + li.LineID + "' AND c.ISDEFAULT = 1 AND c.stopsign = 0 \r"
                    ...
                    //获取线路下标准轨迹数据
                    string queryTrackStr = "SELECT pointSn, lon, lat, direct \r"
                                        + "FROM TB_Line_Tracks \r"
                                        + "WHERE lineID = '" + li.LineID + "' \r"
                                        + "ORDER BY pointSn";
